...
async def handle_vote(session, context, event):
    ...
    try:
        ...
        # Cumulative vote
            ...
            update_poll = await handle_cumulative_vote(
                session, context, event, option, limited=False
            )
...
async def respond_to_vote(
    session, line, context, poll, remaining_votes=None, limited=False
):
...
async def handle_cumulative_vote(session, context, event, option, limited=True):
    ...
    if existing_vote is None and action == CallbackResult.no:
        await respond_to_vote(
            session, "Cannot downvote this option.", context, option.poll
        )
        return False
