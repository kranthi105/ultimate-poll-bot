...
def get_anonymization_confirmation_keyboard(poll):
    ...
    buttons = [
        [
            Button.inline(
                i18n.t("keyboard.permanently_anonymize", locale=locale), data=payload
            )
        ],
...
def get_settings_keyboard(poll):
    ...
    buttons.append(
        [Button.inline(text=allow_new_option_text, data=allow_new_option_payload)]
    )
    ...
    buttons.append([Button.inline(text=allow_sharing_text, data=allow_sharing_payload)])
...
def get_styling_settings_keyboard(poll):
    ...
    if poll.results_visible and not poll.is_priority():
        ...
        buttons.append(
            [Button.inline(text=option_votes_text, data=option_votes_payload)]
        )
    ...
    if poll.is_doodle() or poll.is_priority():
        ...
        buttons.append(
            [Button.inline(text=doodle_button_text, data=doodle_button_payload)]
        )
    ...
    for order in OptionSorting:
        ...
        button = Button.inline(
            i18n.t("keyboard.order_options", locale=locale, name=option_name), data=data
        )
        buttons.append([button])
...
def get_settings_language_keyboard(poll):
    ...
    for language in supported_languages:
        data = (
            f"{CallbackType.settings_change_poll_language.value}:{poll.id}:{language}"
        )
